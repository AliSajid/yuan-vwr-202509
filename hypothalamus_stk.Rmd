...
params:
  chip_type: STK
  signal_file: "kinome_data/STK/Hypothalamus/_ExportstosDB_Median_SigmBg_250907165117.txt" # Change the file name to the one you have
  saturation_file: "kinome_data/STK/Hypothalamus/_ExportstosDB_Signal_Saturation_250907165118.txt" # Change the file name to the one you have
...
qc_data <- krsa_qc_steps(raw_data) |>
    mutate(
        Group = case_when(
            SampleName == "Sedentary + Veh_Hypothalamus" ~ "Sed_Veh",
            SampleName == "Sedentary + EB_Hypothalamus" ~ "Sed_EB",
            SampleName == "VWR + Veh_Hypothalamus" ~ "VWR_Veh",
            SampleName == "VWR + EB_Hypothalamus" ~ "VWR_EB"
        ),
...
comparisons <- list(
    ...
    COMP2 = c(groups[[4L]], groups[[3L]]),
    COMP3 = c(groups[[3L]], groups[[1L]]),
    COMP4 = c(groups[[4L]], groups[[2L]])
)
```
...
case <- comparisons[["COMP2"]][[1L]]
ctrl <- comparisons[["COMP2"]][[2L]]
random <- "B"
threshold <- params[["threshold"]]
...
case <- comparisons[["COMP3"]][[1L]]
ctrl <- comparisons[["COMP3"]][[2L]]
random <- "C"
threshold <- params[["threshold"]]
...
case <- comparisons[["COMP4"]][[1L]]
ctrl <- comparisons[["COMP4"]][[2L]]
random <- "D"
threshold <- params[["threshold"]]
